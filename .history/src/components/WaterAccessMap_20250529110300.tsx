    ...
    <group ref={groupRef} position={position} scale={[scale, scale, scale]}>
      ...
      {[...Array(5)].map((_, i) => (
        <mesh castShadow key={i} position={[
          ...
          (0.6 + Math.random() * 0.3) * scale, // Posiciona acima do caule
          (Math.random() - 0.5) * 0.3 * scale
        ]}>
  ...
  return (
    <group>
      {cloudData.current.map((cloud, i) => (
        <Cloud key={i} {...cloud} opacity={0.6} speed={0.05} bounds={[cloud.scale[0] * 2, cloud.scale[2], cloud.scale[1]]} segments={20} color="#ffffff" />
      ))}
    </group>
  );
};

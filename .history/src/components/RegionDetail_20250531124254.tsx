...
const RegionDetail: React.FC<RegionDetailProps> = ({ region }) => {
  ...
  const getAccessLevelColor = (level: number): string => {
    if (level >= 80) return 'text-green-600/60';
    if (level >= 60) return 'text-blue-600/60';
    if (level >= 40) return 'text-yellow-600/60';
    if (level >= 20) return 'text-orange-600/60';
    return 'text-red-600/60';
  };
